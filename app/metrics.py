...
# histogram_quantile(0.99, sum(rate(publication_integrations_latency_seconds[1m])) by (le, integration))
# среднее время обработки за 1 мин
INTEGRATIONS_LATENCY = prometheus_client.Histogram(
    'publication_integrations_latency_seconds',
    '',
    ['integration'],
    buckets=DEFAULT_BUCKETS,
)
# histogram_quantile(0.99, sum(rate(publication_routes_latency_seconds_bucket[1m])) by (le, endpoint))
# среднее время обработки за 1 мин
ROUTES_LATENCY = prometheus_client.Histogram(
    'publication_routes_latency_seconds',
    '',
    ['method', 'endpoint'],
    buckets=DEFAULT_BUCKETS,
)
